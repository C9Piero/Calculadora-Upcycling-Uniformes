...
with col1:
    ...
    if st.button("➕ Agregar al reporte"):
        ...
        st.rerun() # Esto es clave: obliga a la app a actualizarse y mostrar los nuevos totales
...
with col2:
    if st.session_state.data:
        ...
        m2.markdown(f'<div class="metric-card"><div class="metric-val">💧 {int(tot_agua):,} L</div></div>', unsafe_allow_html=True)
        m3.markdown(f'<div class="metric-card"><div class="metric-val">🚗 {int(tot_co2/0.25):,} km</div></div>', unsafe_allow_html=True)
